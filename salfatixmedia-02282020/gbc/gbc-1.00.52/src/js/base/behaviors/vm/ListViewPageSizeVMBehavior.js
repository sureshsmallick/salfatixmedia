  ...
  function(context, cls) {
    ...
    cls.ListViewPageSizeVMBehavior = context.oo.Singleton(cls.BehaviorBase, function($super) {
      return /** @lends classes.ListViewPageSizeVMBehavior.prototype */ {
        ...
        _apply: function(controller, data) {
          ...
          if (this._checkListViewSupport(controller, data)) {
            ...
            for (lineIndex = 0; lineIndex < bufferSize; ++lineIndex) {
              ...
              var rowWidget = cls.WidgetFactory.createWidget("ListViewRow", tableWidget.getBuildParameters());
              tableWidget.addChildWidget(rowWidget);

              // Add widgets for first column
              if (tableColumns.length > 0) {
                this._createControllersAndWidgets(tableColumns[0], lineIndex, tableWidget, 0);
              }
              // Add widgets for second column
              if (tableColumns.length > 1) {
                this._createControllersAndWidgets(tableColumns[1], lineIndex, tableWidget, 1);
              }
            }
            tableWidget.updateHighlight();
          }
        },
        ...
        _createControllersAndWidgets: function(node, lineIndex, tableWidget, columnIndex) {
          ...
          if (valueList) {
            ...
            if (widget) {
              var row = tableWidget.getChildren()[lineIndex];
