      ...
      return /** @lends classes.TableColumnTitleWidget.prototype */ {
        ...
        _buildContextMenu: function(domEvent) {
          ...
          if (table) {
            ...
            // Autoset column width based on values
            var doAllColumnAutoWidth = cls.WidgetFactory.createWidget("Label", opts);
            doAllColumnAutoWidth.setValue(i18next.t("gwc.contextMenu.doAllColumnAutoWidth"));
            doAllColumnAutoWidth.addClass("gbc_doAllColumnAutoWidth_action");
            this._contextMenu.addChildWidget(doAllColumnAutoWidth, {
              clickCallback: function() {
                this._contextMenu.hide();
                this.getTableWidget().autoSetColumnWidths();
              }.bind(this)
            });

            // Autoset column width so all columns visible
            ...
            var seeAllVisibleColumnLabel = cls.WidgetFactory.createWidget("Label", opts);
            seeAllVisibleColumnLabel.setValue(i18next.t("gwc.contextMenu.seeAllVisibleColumn"));
            seeAllVisibleColumnLabel.addClass("gbc_seeAllVisibleColumn_action");
            this._contextMenu.addChildWidget(seeAllVisibleColumnLabel, {
              clickCallback: function() {
                this._contextMenu.hide();
                this.getTableWidget().autoSetColumnWidthsToBeAllVisible();
              }.bind(this)
            });
            ...
            this._element.domFocus(null, table.getElement()); // beware setFocus should not raise a scroll event (it will immediatly close contextmenu)
         ...
         * @returns {classes.ChoiceDropDownWidget} the context menu widget
         */
