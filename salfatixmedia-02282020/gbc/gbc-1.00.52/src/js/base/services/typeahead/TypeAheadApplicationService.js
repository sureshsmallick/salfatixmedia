      ...
      return /** @lends classes.TypeAheadApplicationService.prototype */ {
        ...
        _addCommand: function(cmd) {
          if (this._application) {
            context.LogService.typeahead.log("[app" + this._application.applicationHash + "] " + cmd.__name + " command added", cmd);
            ...
            if (this._currentGroupCommandQueue.length > 0) { // if there is a current group command is added to this group
              ...
              this._registerTimeout(function() { // don't use requestAnimationFrame because when a new tab is opened, animationframe is never called
        ...
        hasPendingNavigationCommands: function() {
          for (var i = 0; i < this._commandsQueue.length; ++i) {
            var cmd = this._commandsQueue[i];
            if ((cmd instanceof cls.TypeAheadCurrentRow || cmd instanceof cls.TypeAheadFocus) && (cls.ActionNode.isTableNavigationAction(
