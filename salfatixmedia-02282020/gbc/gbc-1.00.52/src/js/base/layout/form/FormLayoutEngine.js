      ...
      return /** @lends classes.FormLayoutEngine.prototype */ {
            ...
            if (windowWidget.getLayoutInformation().isSizable()) {
              ...
              if (modal._hasBeenSized) {
                ...
                formMeasured.setWidth(Math.max(childPreferred.getWidth(true), sizedX - window.scrollBarSize, childMinimal.getWidth(
                  true)));
                formMeasured.setHeight(Math.max(childPreferred.getHeight(true), sizedY - window.scrollBarSize, childMinimal.getHeight(
                  true)));
              }
        ...
        _notify: function() {
          ...
          if (widget) {
            ...
            if (windowWidget && !windowWidget._disabled) {
              ...
              style[".g_measured #w_" + this._widget.getUniqueIdentifier() + ".g_measureable>.containerElement>*"] = {
                width: (childAllocated && childAllocated.hasWidth(true)) ? cls.Size.cachedPxImportant(childAllocated.getWidth()) :
                  null,
                height: (childAllocated && childAllocated.hasHeight(true)) ? cls.Size.cachedPxImportant(childAllocated.getHeight()) :
                  null
              };
              styler.appendStyleSheet(style, this._styleSheetId, true, this.getLayoutSheetId());
        ...
        _notifyModal: function() {
          ...
          if (modalcontent) {
            ...
            styler.appendStyleSheet(minStyle, "formLayout_" + modal.getUniqueIdentifier(), true, this.getLayoutSheetId());
            ...
            style[".g_measured .gbc_ModalWidget #w_" + this._widget.getUniqueIdentifier() + ".g_measureable"] = {
              width: cls.Size.cachedPxImportant(modal._hasBeenSized ? modalContentWidth : (measure.getWidth() + window.scrollBarSize)),
              height: cls.Size.cachedPxImportant(modal._hasBeenSized ? modalContentHeight : (measure.getHeight() + window.scrollBarSize))
            };
            ...
            if (windowWidget._toolBarWidget) {
              windowWidget._toolBarWidget.setStyle({
                "width": (modal._hasBeenSized ? modalContentWidth : contentMaxWidth) + "px"
              });
            }
