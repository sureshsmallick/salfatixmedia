          ...
          orderFields: function(node, fields, nodeTypes, attributesFilter) {
            ...
            if (ok) {
              if (fields.length === 0) {
                ...
                var nodeTabIndex = node.attribute('tabIndexRt');
                var inserted = false;
                for (i = 0; i < fields.length; ++i) {
                  var field = fields[i];
                  var fieldTabIndex = 0;
                  if (field.getTag() === "Table") {
                    fieldTabIndex = field.getFirstChild('TableColumn').attribute('tabIndexRt');
                  } else {
                    fieldTabIndex = fields[i].attribute('tabIndexRt');
                  }
                  if (fieldTabIndex > nodeTabIndex) {
        ...
        restoreVMFocus: function(restoreDOMFocus) {
          ...
          if (node) {
            ...
            if (ctrl) {
              ...
              if (uiWidget && (restoreDOMFocus || uiWidget.hasVMFocusedWidgetChanged())) {
                // if no visibleId is set we make sure to display potential potential current page
