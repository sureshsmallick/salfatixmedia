...
(function() {
  ...
  Element.prototype.domFocus = function(callback, noScrollContainer) {
    ...
    // if modal system is opened we should not change the focus
    if (gbc.systemModalOpened === false && this !== document.activeElement) {
   ...
   * @returns {boolean}
   */
  Element.prototype.isInDOMBody = function() {
    var found = false,
      parent = this.parentNode;
    while (!found) {
      if (!parent) {
