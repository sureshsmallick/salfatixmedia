...
var OpAttributeSanitizer = (function () {
    ...
    OpAttributeSanitizer.sanitize = function (dirtyAttrs) {
        ...
        var font = dirtyAttrs.font, size = dirtyAttrs.size, link = dirtyAttrs.link, script = dirtyAttrs.script, list = dirtyAttrs.list, header = dirtyAttrs.header, align = dirtyAttrs.align, direction = dirtyAttrs.direction, indent = dirtyAttrs.indent, mentions = dirtyAttrs.mentions, lineheight = dirtyAttrs.lineheight;
    ...
    OpAttributeSanitizer.IsValidSize = function (size) {
        return !!size.match(/^[0-9]*px|[a-z\-]{1,20}$/i);
    };
...
var OpToHtmlConverter = (function () {
    ...
    OpToHtmlConverter.prototype.getCssStyles = function () {
        var attrs = this.op.attributes;
        var propsArr = [['color'], ['size'], ['font'], ['align'], ['indent'], ['lineheight']];
        if (!this.options.allowBackgroundClasses) {
            propsArr.push(['background', 'background-color']);
        }
        if(attrs.indent){
            // convert indent lvl to padding em
            var indents = [0,3,6,9,12,15,18,21,24,27];
            attrs.indent = indents[attrs.indent]+"em";
        }
        return propsArr
            .filter(function (item) { return !!attrs[item[0]]; })
            .map(function (item) { return item._preferSecond()
                .replace("font","font-family")
                .replace("align","text-align")
                .replace("indent","padding-left")
                .replace("lineheight","line-height")
                .replace("size","font-size") + ':' + attrs[item[0]]; });
    };
    OpToHtmlConverter.prototype.getTagAttributes = function () {
        var checkClassesToInline= function(n){return ["align","indent"].indexOf(n)>=0};
        ...
        if (this.op.isFormula() || (this.op.isContainerBlock() && (this.op.attributes && !checkClassesToInline("align") && !checkClassesToInline("indent")))) {
            return tagAttrs;
        }
        if (this.op.isVideo()) {
...
function encodeHtml(str, preventDoubleEncoding) {
    ...
    return str
        ...
        .replace(/'/g, "&#x27;")
        .replace(/\//g, "&#x2F;")
        .replace(/\t/g, "\u00a0\u00a0\u00a0\u00a0") // replace tabs by 4 non-breaking spaces
        .replace(/  /g, " &nbsp;")
        .replace(/ &nbsp;/g, "&nbsp;&nbsp;")
        .replace(/&nbsp; /g, "&nbsp;&nbsp;");
}
